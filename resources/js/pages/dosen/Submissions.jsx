                                        ...
                                        <tbody>
                                            {taskSubs.map((s, i) => (
                                                <tr key={s.id} style={{ borderBottom: i === taskSubs.length - 1 ? "none" : "1px solid #f1f5f9", transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                                                    ...
                                                    <td style={{ padding: "20px" }}>
                                                        {s.file
                                                            ? <a href={`${import.meta.env.VITE_API_BASE_URL}/storage/${s.filePath}?download=1`} download={s.file} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 700, color: "#4f46e5", background: "#eef2ff", padding: "6px 12px", borderRadius: 10, textDecoration: "none", transition: "all 0.2s", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 180 }} onMouseEnter={e => e.currentTarget.style.background = "#e0e7ff"} onMouseLeave={e => e.currentTarget.style.background = "#eef2ff"}><FileText size={14} />{s.file}</a>
